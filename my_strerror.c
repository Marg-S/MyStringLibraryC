...
char *my_strerror(int errnum) {
  char *str = my_NULL;
  if (errnum >= 0 && errnum <= MAX_ERR_N)
    str = (char *)my_errors[errnum];
  else {
    static char str_unknown[32] = {'\0'};
    my_sprintf(str_unknown, "%s%d", my_errors[MAX_ERR_N + 1], errnum);
    str = str_unknown;
  }
  return str;
}
